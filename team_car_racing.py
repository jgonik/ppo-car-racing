...
class TeamCarRacing(gym.Env, EzPickle):
    metadata = {
        'render.modes': ['human', 'rgb_array', 'state_pixels'],
        'video.frames_per_second' : FPS
    }

    def __init__(self, num_teams=2, num_agents_per_team=2, verbose=1, direction='CCW',
                 use_random_direction=True, backwards_flag=True, h_ratio=0.25,
                 use_ego_color=False):
        EzPickle.__init__(self)
        self.seed()
        self.total_num_agents = num_agents_per_team * num_teams
        self.num_teams = num_teams
        self.num_agents_per_team = num_agents_per_team
        self.contactListener_keepref = FrictionDetector(self)
        self.world = Box2D.b2World((0,0), contactListener=self.contactListener_keepref)
        self.viewer = [None] * self.total_num_agents
        self.score_label = [None] * self.total_num_agents
    ...
    def _create_track(self):
        ...
        for i in range(len(track)):
            ...
            t.road_visited = [False] * self.total_num_agents
            t.road_friction = 1.0
    ...
    def reset(self):
        ...
        # Set positions of cars randomly while keeping teams together
        # ids = [i for i in range(self.total_num_agents)]
        # groups = [list(g) for _, g in itertools.groupby(ids, lambda x: x // self.num_agents_per_team)]
        # random.shuffle(groups)
        # shuffled = [item for group in groups for item in group]
        # self.car_order = {i: shuffled[i] for i in range(len(shuffled))}

        # Set positions of cars randomly
        ids = [i for i in range(self.total_num_agents)]
        shuffle_ids = np.random.choice(ids, size=self.total_num_agents, replace=False)
        self.car_order = {i: shuffle_ids[i] for i in range(self.total_num_agents)}
        ...
        for car_id in range(self.total_num_agents):
    ...
    def step(self, action):
        ...
        step_reward = np.zeros(self.total_num_agents)
        done = False
    ...
    def render(self, mode='human'):
        ...
        result = []
        for cur_car_id in range(self.total_num_agents):
    ...
    def _render_window(self, car_id, mode):
        ...
        if self.viewer[car_id] is None:
            ...
            self.score_label[car_id] = pyglet.text.Label('0000', font_size=36,
...
if __name__=="__main__":
    ...
    # Supports key control of four cars, but can simulate as many as needed (teams of two)
    NUM_CARS_PER_TEAM = 2
    NUM_TEAMS = 2
    ...
    CAR_CONTROL_KEYS = [[key.LEFT, key.RIGHT, key.UP, key.DOWN],
                        ...
                        [key.F, key.H, key.T, key.G]]

    a = [np.zeros((NUM_CARS_PER_TEAM,3)) for _ in range(NUM_TEAMS)]
    def key_press(k, mod):
        ...
        for i in range(min(len(CAR_CONTROL_KEYS), NUM_CARS_PER_TEAM * NUM_TEAMS)):
            team = i // NUM_CARS_PER_TEAM
            car_index = i % NUM_CARS_PER_TEAM
            if k==CAR_CONTROL_KEYS[i % len(CAR_CONTROL_KEYS)][0]: a[team][car_index][0] = -1.0
            if k==CAR_CONTROL_KEYS[i % len(CAR_CONTROL_KEYS)][1]: a[team][car_index][0] = +1.0
            if k==CAR_CONTROL_KEYS[i % len(CAR_CONTROL_KEYS)][2]: a[team][car_index][1] = +1.0
            if k==CAR_CONTROL_KEYS[i % len(CAR_CONTROL_KEYS)][3]: a[team][car_index][2] = +0.8   # set 1.0 for wheels to block to zero rotation
    ...
    def key_release(k, mod):
        ...
        for i in range(min(len(CAR_CONTROL_KEYS), NUM_CARS_PER_TEAM * NUM_TEAMS)):
            team = i // NUM_CARS_PER_TEAM
            car_index = i % NUM_CARS_PER_TEAM
            if k==CAR_CONTROL_KEYS[i % len(CAR_CONTROL_KEYS)][0] and a[team][car_index][0]==-1.0: a[team][car_index][0] = 0
            if k==CAR_CONTROL_KEYS[i % len(CAR_CONTROL_KEYS)][1] and a[team][car_index][0]==+1.0: a[team][car_index][0] = 0
            if k==CAR_CONTROL_KEYS[i % len(CAR_CONTROL_KEYS)][2]: a[team][car_index][1] = 0
            if k==CAR_CONTROL_KEYS[i % len(CAR_CONTROL_KEYS)][3]: a[team][car_index][2] = 0


    env = TeamCarRacing(NUM_TEAMS, NUM_CARS_PER_TEAM)
    env.render()
    ...
    while isopen and not stopped:
        env.reset()
        total_reward = np.zeros(NUM_CARS_PER_TEAM * NUM_TEAMS)
        steps = 0
        restart = False
        while True:
            ...
            if steps % 200 == 0 or done:
                #FIXME(IG): The total reward seems to be shown wrongly.
                print("\nActions: " + str.join(" ", [f"Team {team}, Car {car_index}: "+str(a[team][car_index]) for team in range(NUM_TEAMS) for car_index in range(NUM_CARS_PER_TEAM)]))
